...
export function ContentStudio({ session, onLogout, onSessionExpired }) {
  ...
  if (showInbox) return <main className="studio-page"><header className="studio-toolbar"><div><p className="eyebrow">Private workspace</p><h1>Contact inbox</h1></div><div><button onClick={() => setShowInbox(false)}>Back to content</button><button onClick={onLogout}>Sign out</button></div></header><section className="message-inbox"><div className="inbox-heading"><div><h2>Messages</h2><p>{messages.filter((message) => message.status === 'new').length} new</p></div><button onClick={openInbox}>Refresh</button></div>{!messages.length ? <div className="empty-editor"><h2>No messages yet</h2><p>New contact-form submissions will appear here.</p></div> : <div className="message-list">{messages.map((message) => <article className={`message-card ${message.status}`} key={message.id}><header><div><strong>{message.first_name} {message.last_name}</strong><a href={`mailto:${message.email}`}>{message.email}</a></div><time>{new Date(message.created_at).toLocaleString()}</time></header><dl>{message.company && <><dt>Company</dt><dd>{message.company}</dd></>}{message.title && <><dt>Title</dt><dd>{message.title}</dd></>}{message.work_phone && <><dt>Work phone</dt><dd><a href={`tel:${message.work_phone}`}>{message.work_phone}</a></dd></>}</dl>{message.message && <p>{message.message}</p>}<footer><select aria-label="Message status" value={message.status} onChange={(event) => updateMessage(message.id, event.target.value)}><option value="new">New</option><option value="read">Read</option><option value="archived">Archived</option></select><button className="danger-action" onClick={() => deleteMessage(message.id)}>Delete</button></footer></article>)}</div>}</section></main>
